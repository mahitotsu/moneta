...
import { backdateSagaUpdatedAt, getSaga, waitForOwnerIndexed, waitForSagaState } from "../support/sagaState";
import { openFreshAccount } from "../support/testAccount";
import { confirmTransfer, recallTransfer, startTransfer } from "../support/transferClient";
...
async function creditedFurikomi(commandApi: ReturnType<typeof createCommandApi>, queryApi: ReturnType<typeof createQueryApi>, outputs: Awaited<ReturnType<typeof fetchStackOutputs>>) {
  ...
  const transferId = `e2e-recall-source-${crypto.randomUUID()}`;
  await startTransfer(outputs.transferCommandQueueUrl, { transferId, fromAccountId: fromId, toAccountId: toId, amount: "300.00" });
  await waitForSagaState(outputs.transferSagaTableName, transferId, ["pending_confirmation"]);
  await confirmTransfer(outputs.transferCommandQueueUrl, transferId);
  await waitForSagaState(outputs.transferSagaTableName, transferId, ["credited"]);
...
describe("J9: 期限内のCredited済み振込は組戻しできる", () => {
  it("受取人から送金元へ全額が戻る", async () => {
    ...
    const { fromId, toId, transferId } = await creditedFurikomi(commandApi, queryApi, outputs);

    const recallId = `e2e-recall-${crypto.randomUUID()}`;
    await recallTransfer(outputs.transferCommandQueueUrl, { transferId: recallId, originalTransferId: transferId });

    const saga = await waitForSagaState(outputs.transferSagaTableName, recallId, ["credited"]);
    expect(saga.kind).toBe("recall");
    // 組戻しは受取人→送金元の逆方向送金として実装される(docs/adr/0011決定5)。
    expect(saga.fromAccountId).toBe(toId);
    expect(saga.toAccountId).toBe(fromId);
...
describe("J10a: 受取人の残高不足での組戻しはfailedになる", () => {
  it("組戻し用サガがfailedになり、双方の残高は組戻し前のまま変化しない", async () => {
    ...
    const recallId = `e2e-recall-insufficient-${crypto.randomUUID()}`;
    await recallTransfer(outputs.transferCommandQueueUrl, { transferId: recallId, originalTransferId: transferId });

    const saga = await waitForSagaState(outputs.transferSagaTableName, recallId, ["failed"]);
...
describe("J10b: 組戻し可能な時間窓を過ぎている場合は受付時点で却下される", () => {
  it("saga.rsのRECALL_WINDOW(24時間)を過ぎたサガへのRecallはWindowExpiredで却下される", async () => {
    ...
    const recallId = `e2e-recall-expired-${crypto.randomUUID()}`;
    await recallTransfer(outputs.transferCommandQueueUrl, { transferId: recallId, originalTransferId: transferId });

    await settle(REJECTION_SETTLE_MS);

    const recallSaga = await getSaga(outputs.transferSagaTableName, recallId);
    expect(recallSaga).toBeNull();
