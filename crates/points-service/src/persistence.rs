...
/// (該当すれば)アウトボックスPut]の順に積むため、reasons[0]が条件不成立なら「重複配信」、
/// reasons[1]が条件不成立なら楽観ロックの競合としてリトライ対象にする。
...
/// アウトボックスへのPutは行わない——結果を待つ呼び出し元がいないため。
pub async fn credit_points(
    ...
    let items = vec![idempotency_put(&tables.idempotency, idempotency_key, now), points_write(&tables.points, owner_id, new_balance, current_version)];
