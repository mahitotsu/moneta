...
サガ状態はQuery serviceと同じ技術選択(DynamoDB on-demand)で持つ。理由は「Query serviceと
揃えたいから」ではなく、この用途に固有の判断による。

- サガの状態更新は常に「1つの`transfer_id`につき1アイテム」の読み書きで完結し、複数行に
  またがるトランザクションは発生しない。DynamoDBの条件付き書き込み(`ConditionExpression`)で
  十分に表現でき、DSQLのリレーショナル・トランザクション機構は使われないまま残る。
- DSQLを新たに使うなら、[[0005-automated-schema-migration-custom-resource]]のスキーマ自動適用の
  仕組み(CDK Custom Resource + マイグレーターLambda)をtransfer-service用にもう一セット
  必要とする。サガの項目は数フィールド程度でスキーマレスな用途に十分収まるため、この重さに
  見合わない。
- なお、コストは決定打ではない。[[0004-query-service-event-driven-projection]]の監査で確認した
  通り、Aurora DSQL自体はアイドル時DPU課金ゼロであり、DynamoDBだけが「アイドル時無料」という
  わけではない。
...
- **2相コミット/XA的な分散トランザクション**: Aurora DSQLはそもそもクロスaggregateの
  トランザクションをサポートせず、イベント駆動の設計思想全体とも相容れないため検討していない。
...
- **顧客向けコマンドAPI([[0006-write-path-api-gateway-sqs-direct-integration]])経由でコマンドを
  ...
  避けた(決定1)。
- **サガ状態専用の新しいAurora DSQLクラスタを持つ**: コストは排除されない(DSQLもアイドル時
  DPU課金ゼロ)が、単一アイテムの読み書きしか発生しないサガ状態にリレーショナル・トランザクション
  機構は不要であり、ADR-0005相当のスキーマ自動適用の仕組みをもう一セット持つ重さに見合わない
  ため見送った(決定2)。
