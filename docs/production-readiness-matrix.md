...
| # | 項目 | 判定 | 根拠 |
|---|---|---|---|
| S1 | 認証・認可・セッション管理 | 🟡(意図的にスコープ外と明記) | [ADR-0007](adr/0007-web-ui-stack-and-hosting.md)/[ADR-0009](adr/0009-web-ui-customer-experience-and-channel-emulation.md)「認証UIなし」 |
...
| S4 | 敵対的input耐性(異常に大きい金額・長い文字列・大量リクエストでのふるまい) | 🔴🚧**ブロック中** | ADR・テストともに言及なし。**テスト以前に「どこまでをサポートする振る舞いとして定義するか」の設計判断(ADR)が先に必要**(例: 金額の桁数上限、APIGWレベルのリクエストサイズ制限、レート制限の要否)。この判断なしにテストを書いても、何を「正しい」とみなすかの基準がない |
...
| D3 | 資金移動: 口座間(振替・振込・組戻し) | 🟢 | ⓪FC10-FC12、[[0010-transfer-service-saga]]/[[0011-furikae-furikomi-distinction]] |
...
パターンの本質(補償・確認要否)は変わらないと想定されるが、明記はされていない |
...
| D7 | 手数料・課金(振込手数料・口座維持手数料) | 🔵 将来の検証候補 | 同上、0件(誤検出を除く)。イベント購読×補償サガという既存パターンに最も近い |
...
| D13 | KYC・本人確認・口座開設審査 | 🟡 | [[0007-web-ui-stack-and-hosting]]/[[0009]]「認証UIなし・ダミーサインイン」が
実質的にこの領域全体を明示的にスコープ外としている |
| D14 | 認証・認可・セッション管理 | 🟡 | ①S1と同一(重複整理) |
...
- **D5(定期振込)・D6(金利計算)・D7(手数料)・D16(休眠口座)・D17(規制報告)** →
...
**⚪(D8/D9/D10)についてのみ、その判断をCLAUDE.mdか新規ADRとして明文化する価値が残っている**。
🔵(D5/D6/D7/D16/D17)は「対象外」ではなく「将来やる価値がある」という記録なので、明文化の
必要はなく、このマトリクス自体がバックログとして機能する。
...
6. **(新規)④のD8・D9・D10(3件)について、「技術検証の対象外」という判断をCLAUDE.mdまたは新規ADRとして明文化する**——実装追加ではなく文書化のみで解消する項目
6b. **(新規・実装着手は別途合意)** D5・D6・D7・D16・D17(🔵将来の検証候補)は本マトリクスにバックログとして記録済み。着手する場合は個別に優先順位を合意する
