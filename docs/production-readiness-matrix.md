...
| # | 項目 | 判定 | 根拠 |
|---|---|---|---|
| S1 | 認証・認可・セッション管理 | 🟢(item単位の読み取り認可を除く) | [ADR-0016](adr/0016-cognito-authentication.md): Amazon Cognito User Poolによる実サインアップ/サインイン、顧客向けエンドポイント(`Deposit`/`Withdraw`を除く)全てに`CognitoUserPoolsAuthorizer`、`owner_id`はJWTの`sub`が正(自己申告を無視)、`Freeze`/`Unfreeze`/`Close`は`requested_by`と現在の`owner_id`の一致を検証(`DomainError::NotOwner`)。**残存ギャップ(ADR-0016トレードオフ節に明記)**: 認証済みの別人が他人の`accountId`を直接指定すれば`GET /accounts/{id}`は依然として閲覧できる——書き込み系は認可済みだが読み取り系は認証止まり、という非対称が残る(Lambdaレス直接統合という設計上、item単位の認可をVTLで表現しづらいため)。同じ非対称は`GET /transfers/{transferId}`(`TransferQueryApi`)にもある——2026-08-18、[[0025-points-balance-query-api-and-header-display]]の動作確認中に無関係な使い捨て識別子から他人の送金を実際に閲覧できることを直接確認した。`GET /customers/me/points`(`PointsQueryApi`、決定1)はJWTの`sub`から`ownerId`を導出する形(`accountId`のようなクライアント指定のIDを取らない)のため、この種のitem単位の認可漏れは構造的に発生しない |
