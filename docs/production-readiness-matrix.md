...
| P2 | 実運用に近い流量(多数口座が同時に取引)での挙動 | 🔴🚧**ブロック中** | 全ての並行性テストが「1口座に対し2〜10件」のみ。多数の異なる`MessageGroupId`が同時に走る状況(Lambdaの同時実行数上限、スケーリング特性)は一度も検証されていない。**負荷生成ツール(k6・Artillery等)を新たに導入し、デプロイ済みの実環境に対して数百口座・数千リクエスト規模で駆動する仕組みが要る**——`api-e2e`の既存のJestベースの仕組みでは非現実的な規模。I1(持続的インフラ障害)と同じ「手動/オンデマンド、実環境限定」という位置づけになる見込み。**参考データ点(2026-08-21、③節末尾「関連する調査結果」)**: `api-e2e`フルスイート並列実行(938件のaccount-service呼び出し、同時実行数最大17)ではLambda/DynamoDB/SQS/Cognitoのいずれもスロットリング0件だった——本格的なk6ベースの負荷試験の代わりにはならないが、少なくともこの規模では劣化の兆候がないことを示す |
...
| # | 項目 | 判定 | 根拠 |
|---|---|---|---|
| L1 | 資金保存則(システム全体で合計金額が保存される) | 🟢(2026-08-10、`fast-check`を導入し`conservation-property.e2e.test.ts`としてプロパティベーステストを実装。ライブスタックに対して実行・合格確認済み、2026-08-12、numRuns=5全通過。2026-08-21、`api-e2e`フルスイート並列実行時に本テストだけが単体実行の15倍でタイムアウトする事象を調査・修正——詳細は本節末尾の「関連する調査結果」参照) | 同一名義口座間の振替が閉じた系の合計金額を変えないことをランダムな操作列で検証 |
...
### 関連する調査結果(2026-08-21): L1のタイムアウトはAWS側の性能劣化ではなかった

`api-e2e`フルスイート(29ファイル、`--maxWorkers=3`で並列実行)を通しで流すと、
`conservation-property.e2e.test.ts`(L1)だけが単体実行の約15倍(約100秒→900秒超)で
恒久的にタイムアウトする事象が発生した——「イベント駆動アーキテクチャがスケールしない」
ことの証拠に見えかねない数字だったため、CloudWatchで実際のAWS側の挙動を調査した。

該当時間帯(2026-08-21 13:25〜13:44 UTC)の実測:

| 指標 | 実測値 |
|---|---|
| Lambda Throttles(全関数合計) | 0件 |
| Lambda同時実行数の最大 | 17(デフォルト上限1000に対し無視できる規模) |
| account-serviceの処理時間 | 平均30〜120ms、劣化なし |
| DynamoDBのThrottledRequests(全テーブル) | 0件 |
| DynamoDB書き込み消費量の最大 | 793 WCU/5分(オンデマンドテーブルの上限に対し誤差レベル) |
| SQSの滞留(ApproximateAgeOfOldestMessage)の最大 | 79秒(通常の結果整合性の範囲内) |
| Cognitoスロットリング | 0件 |

**AWS側は938件のaccount-service呼び出しを含む全負荷を、終始スロットリングなし・数十msの
レイテンシで処理していた**。つまりこのタイムアウトはインフラのスケーラビリティ問題ではない。

実際の原因はテスト側の設計だった: `conservation-property.e2e.test.ts`は`fast-check`の
`fc.assert`を使ったプロパティベーステストで、既定では失敗時に**自動shrink**(最小の反例を
探すため、同じ高コストなpredicate——口座4つ開設+最大15件送金+終端状態待ち、を伴う実AWS
呼び出し——を何度も再実行する)が有効になっていた。他スイートとの並列実行下で1回の
`waitFor`(既定30秒)がわずかに間に合わなかった際、このshrinkが同じpredicateを繰り返し
再実行し(1回あたり最大`timeout`の180秒)、900秒という値はこの180秒が4〜5回積み重なった
規模とほぼ一致する——**インフラの限界ではなく、プロパティベーステストのshrink機構が実AWS
呼び出しの失敗を非線形に増幅させていた**。

`fc.noShrink()`でこのテストのshrinkを無効化して修正し、同じ条件(29ファイル並列)で
再実行したところ、**全29スイート・63テストが340秒で完走**(修正前は900秒超で2件失敗)。
`docs/insights.md`§3.5にこの教訓を記録した。
