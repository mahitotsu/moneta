...
| FC20(docs/adr/0024・0025) | `scenarios/transfer-fee-and-points.e2e.test.ts`, `scenarios/points-query.e2e.test.ts` | 実装済み(振込の手数料徴収・ポイント充当・`PendingDebit`却下時の返却、`GET /customers/me/points`の未獲得時0円・未認証401・実際の付与反映。`Compensating`経路でのポイント返却は未検証、`docs/decision-tables.md`発見6参照) |
